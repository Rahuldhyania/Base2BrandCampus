...
import SingleknowledgeCenterContent from '@/app/Components/Page-Components/SingleknowledgeCenterContent';
import { fetchKnowledgeCenterBySlug } from '@/app/Data/Articlesdata';

const KnowledgeCenterDetail = () => {
    const params = useParams();
    const slug = params?.slug;
    const [knowledgeCenter, setKnowledgeCenter] = useState(null);
    ...
    useEffect(() => {
        const loadKnowledgeCenter = async () => {
            ...
            try {
                setLoading(true);
                setError(null);
                const res = await fetchKnowledgeCenterBySlug(slug);
                setKnowledgeCenter(res?.knowledgeCenter || null);
            } catch (err) {
    ...
    return (
        <div>
            <head>
                <title>{knowledgeCenter?.pageTitle}</title>
                <meta name="description" content={knowledgeCenter?.pageDescription} />
            </head>
            <BlogDetailsHead backbtn="Back to Articles" share_btn="Share" backUrl="/knowledge-center" />
            <SingleknowledgeCenterContent knowledgeCenter={knowledgeCenter} />
...
export default KnowledgeCenterDetail;
